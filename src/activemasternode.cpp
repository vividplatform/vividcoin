...
// Copyright (c) 2017-2018 The Vivid developers
...
//
// Bootup the Masternode, look for a VIVID collateral input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if (status != ACTIVE_MASTERNODE_STARTED) {
        ...
        //     if (service.GetPort() != 38079) {
        //         notCapableReason = strprintf("Invalid port: %u - only 38079 is supported on mainnet.", service.GetPort());
        //         LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
        //         return;
        //     }
        // } else if (service.GetPort() == 38079) {
        //     notCapableReason = strprintf("Invalid port: %u - 38079 is only supported on mainnet.", service.GetPort());
        //     LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
        //     return;
        // }
...
bool CActiveMasternode::Register(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& errorMessage)
{
    ...
    // if (Params().NetworkID() == CBaseChainParams::MAIN) {
    //     if (service.GetPort() != 38079) {
    //         errorMessage = strprintf("Invalid port %u for masternode %s - only 38079 is supported on mainnet.", service.GetPort(), strService);
    //         LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
    //         return false;
    //     }
    // } else if (service.GetPort() == 38079) {
    //     errorMessage = strprintf("Invalid port %u for masternode %s - 38079 is only supported on mainnet.", service.GetPort(), strService);
    //     LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
    //     return false;
    // }

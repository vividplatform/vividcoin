...
#if defined(NDEBUG)
#error "Vivid cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uvivid) are considered zero fee (for relaying and mining)
...
{
    int64_t nSubsidy = 0;
	int64_t nSubsidyHalvingInterval = 525600;
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 3000000 * COIN; // Premine
    }
    else if (nHeight >= 2 && nHeight <= 500) {
        nSubsidy = 20 * COIN; // PoW phase (early miners)
    }
    else if (nHeight >= 501 && nHeight <= 525600) {
        nSubsidy = 5 * COIN; // Year 1
    }
    else if (nHeight >= 525601 && nHeight <= 1051200) {
        nSubsidy = 4.5 * COIN; // Year 2
    }
    else if (nHeight >= 1051201 && nHeight <= 2102400) {
      nSubsidy = 4.05 * COIN; // Year 3
    }
    else if (nHeight >= 2102401 && nHeight <= 4204800) {
      nSubsidy = 3.645 * COIN; // Year 4
    }
    else if (nHeight >= 4204801 && nHeight <= 16819200) {
      nSubsidy = 3.2805 * COIN; // Year 5
    } else {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // VIVID: We use certain sporks during IBD, so check to see if they are
